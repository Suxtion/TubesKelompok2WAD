...
<div class="container mx-auto px-4 sm:px-8">
    <div class="py-8">
        ...
        <form action="{{ route('barangs.update', $barang->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-md">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_total" id="jumlah_total" value="{{ old('jumlah_total', $barang->jumlah_total) }}" required min="1"
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_tersedia" id="jumlah_tersedia" value="{{ old('jumlah_tersedia', $barang->jumlah_tersedia) }}" required min="0"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4"
                              class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $barang->deskripsi) }}</textarea>
                    @error('deskripsi') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="md:col-span-2">
                    <label for="gambar" class="block text-sm font-medium text-gray-700 mb-1">Ganti Gambar Barang (Opsional)</label>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Kosongkan jika tidak ingin mengganti gambar. Format: JPG, PNG, GIF, SVG. Maks: 2MB.</p>
                    @error('gambar') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    
                    @if($barang->gambar)
                        <div class="mt-4">
                            <p class="text-sm font-medium text-gray-700">Gambar saat ini:</p>
                            <img src="{{ asset('storage/' . $barang->gambar) }}" alt="Gambar {{ $barang->nama_barang }}" class="mt-2 w-32 h-32 object-cover rounded-md border border-gray-200">
                        </div>
                    @endif
                </div>
            </div>
